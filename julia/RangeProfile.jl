...
f0 = 40e3;
# chirpTime = 5e-3;
# chirpPeriod =1.919e-7;
# K = B/chirpTime;                # Chirp rate
# chirpTimeAxis = collect(0:receivePeriod:chirpTime)
# SampleWave = cos.( 2*pi*(f0*(chirpTimeAxis .- chirpPeriod) + 0.5*K*(chirpTimeAxis .- chirpPeriod).^2) );
...
# v_bb = v_anal.*exp.(-j*2*pi*f0*v_rx);
# V_BB = fft(v_bb);
#
